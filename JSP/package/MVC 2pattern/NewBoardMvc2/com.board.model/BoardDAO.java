...
public class BoardDAO {
	...
	// 여기서부터 게시판에 작업한 기능을 하나씩 메소드로 구현하여 추가하면 됨
		...
		// 검색 때, 글의 총 갯수를 가져오는 메소드
		public int getArticleCount (String searchField, String searchWord) {
			
			Connection con = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			int result = 0;  // result에다가 최종 결과 값을 담아서 반환 시킴
		...
		public List<BoardVO> getArticles(int start, int end, String searchField, String searchWord) {
			...
			List<BoardVO> articleList = new ArrayList<BoardVO>(5);
			
			try {
				con = ConnUtil.getConnection();
				String sql = "select * from (select rownum rnum, num, writer, email, "
						+ "subject, pass, regdate, readcount, ref, step, depth, content, "
						+ "ip from (select * from board where " +searchField
						+ " like ? order by ref desc, step asc)) " 
						+ "where rnum >=? and rnum <=?";
			...
			return articleList; // 검색기능 값 반환, 기존에 것에다가 오버로딩 시킴
		}
		...
		public int deleteArticle(int num, String pass) {
			Connection con = null;
			PreparedStatement pstmtSelect = null; //셀렉트 전용
			PreparedStatement pstmtDelete = null; //딜리트전용
			ResultSet rs = null;
			String dbpassword = "";
			...
			try {
				...
				pstmtSelect = con.prepareStatement("select pass from board where num=?");
