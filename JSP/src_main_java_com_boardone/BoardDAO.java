...
public class BoardDAO {
	...
	public void insertArticle(BoardVO article) {
	    ...
	    try {
	        ...
	        if (pstmtMaxNum != null) try { pstmtMaxNum.close(); } catch (SQLException ss) {}
	        if (pstmtUpdate != null) try { pstmtUpdate.close(); } catch (SQLException ss) {}
	        if (pstmtInsert != null) try { pstmtInsert.close(); } catch (SQLException ss) {}
	...
	/*    글 수정시에는 글 목록보기와 다르게 조회수를 증가시킬 필요가 없다
	 *     조회수를 증가시키는 부분을 제외하고, num에 해당하는 게시글만 가져오는 메소드를 구현함
	 */
	...
	/* 글삭제할꼬얌
	 *  글 삭제 버튼을 클릭하여 글 삭제 화면이 나오면 비밀번호를 입력하고 삭제를 처리한다.
	 *  이때, 데이터베이스에서 비밀번호를 비교하여 일치하면 글 삭제 처리를 수행한다.
	 *  
	 */
